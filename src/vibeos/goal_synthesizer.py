...
class OpenAICompatibleGoalSynthesisProvider(GoalSynthesisProvider):
    ...
    def synthesize(self, utterance: str, analysis: UtteranceAnalysis) -> dict[str, object]:
        ...
        if not self.config.configured or not model_guidance_enabled("VIBEOS_ENABLE_MODEL_GOAL_SYNTHESIS"):
            return self._host_hint_fallback(host_hint, error="missing_api_key_or_model_or_guidance_disabled")
        ...
        try:
            ...
            return self._host_hint_fallback(host_hint, error=str(exc))

    def _host_hint_fallback(self, host_hint: dict[str, object], *, error: str) -> dict[str, object]:
        payload = dict(host_hint)
        self._last_parse_valid = False
        self._last_fallback_used = True
        self._last_error = error
        if not payload.get("message"):
            payload["message"] = "goal synthesis provider unavailable; using bounded host hint"
        self._last_raw_output = json.dumps(payload, ensure_ascii=False)
        return payload
