...
def fixture_tool_specs() -> tuple[ToolSpec, ...]:
    ...
    def locate(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        if fixture is None:
            return ToolResult(
                ...
                evidence={"app": app_name},
                failure_class="environment_unreachable",
            )
        if context.environment.dry_run:
            return ToolResult(
                status="succeeded",
                output={"search_control_id": "search_box", "adapter_status": "dry_run"},
                evidence={"app": app_name, "fixture_id": fixture.fixture_id, "control_id": "search_box", "dry_run": True},
        ...
        if not fixture.has_control("search_box"):
            return ToolResult(
                ...
                evidence={"app": app_name, "fixture_id": fixture.fixture_id},
    ...
    def shortcut(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        if context.environment.dry_run:
            return ToolResult(
                status="succeeded",
                output={"search_mode": "shortcut", "adapter_status": "dry_run"},
                evidence={"app": app_name, "fixture_id": fixture.fixture_id, "shortcut": "Ctrl+K", "dry_run": True},
    ...
    def enter(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        if not context.state.get("search_control_id") and context.state.get("search_mode") != "shortcut":
            return ToolResult(
                status="failed",
                message="search query entry requires a visible control or active shortcut mode",
                evidence={"app": app_name},
                failure_class="semantic_mismatch",
            )
        results = () if context.environment.dry_run else fixture.results_for(query)
        return ToolResult(
            status="succeeded",
    ...
    def observe(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        app_name = str(payload.get("app") or "").strip()
        query = str(context.state.get("search_query") or payload.get("query") or "").strip()
        results = tuple(str(item) for item in context.state.get("observed_results", ()))
        interaction_surface = "shortcut" if context.state.get("search_mode") == "shortcut" else "structured"
        adapter_status = "dry_run" if context.environment.dry_run else "succeeded"
        return ToolResult(
            status="succeeded",
            output={
                "status": "dry_run" if context.environment.dry_run else "succeeded",
            ...
            evidence={"app": app_name, "query": query, "observed_results": results},
    ...
    def verify(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        return ToolResult(
            ...
            message="app verifier observed the requested target in search results"
            if accepted
            else "app verifier did not observe the requested target in search results",
            evidence={"query": query, "observed_results": observed},
            accepted=accepted,
