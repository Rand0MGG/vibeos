...
def browser_tool_specs(portal: PortalAdapter, verifiers: VerifierHarness) -> tuple[ToolSpec, ...]:
    def action(tool_id: str, payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        if context.environment.dry_run:
            ...
            return ToolResult(status="succeeded", output={"selected_target": uri, "uri": uri, "adapter": "browser.semantic", "adapter_status": "dry_run"}, evidence={"uri": uri, "query": query, "site": site, "dry_run": True}, state_updates={"selected_target": uri})
        with browser_attempt_scope(run_id=context.goal_id, attempt_id=context.attempt_id, route_id=context.strategy_id):
            result = portal.open_uri(uri)
            if query or site:
                record_browser_navigation(uri=uri, query=query, site=site, adapter=str(result.get("adapter") or "browser.semantic"), status="opened" if result.get("status") == "opened" else str(result.get("status") or "failed"))
        status = str(result.get("status") or "failed")
        if status == "opened":
            return ToolResult(status="succeeded", output={"selected_target": uri, "uri": uri, "adapter": "browser.semantic", "adapter_status": "succeeded", **result}, evidence={"uri": uri, "query": query, "site": site}, state_updates={"selected_target": uri})
        return ToolResult(status="failed", message=str(result.get("error") or "browser action failed"), output={"uri": uri, "adapter": "browser.semantic", "adapter_status": status, **result}, evidence={"uri": uri, "query": query, "site": site}, failure_class="tool_timeout" if status == "timeout" else "environment_unreachable")
    ...
    def observe(_payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        return ToolResult(status="succeeded", output={"observed_url": snapshot.get("active_url"), "observed_query": snapshot.get("query")}, evidence=snapshot, state_updates={"observed_url": snapshot.get("active_url"), "observed_query": snapshot.get("query")})
    ...
    def resolve_named(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        if not resolved:
            return ToolResult(status="failed", message="no local direct-open resolution matched the named website target", evidence={"requested_name": name}, failure_class="semantic_mismatch")
        return ToolResult(status="succeeded", output={"resolved_url": resolved}, evidence={"requested_name": name, "resolved_url": resolved, "resolution_source": "local_catalog"}, state_updates={"resolved_url": resolved})
    ...
    def open_resolved(_payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        return action("browser.open_url", {"uri": resolved}, context) if resolved else ToolResult(status="failed", message="named browser target has not been resolved", failure_class="semantic_mismatch")
    ...
    def observe_search(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        return ToolResult(status="succeeded", output={"official_result_url": official_url, "result_count": 1 if official_url else 0}, evidence={"query": query, "official_result_url": official_url, "result_source": "local_catalog"}, state_updates={"official_result_url": official_url})
    ...
    def follow_search(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        return action("browser.open_url", {"uri": url}, context) if url else ToolResult(status="failed", message="browser search results did not provide a follow-up destination", evidence={"query": payload.get("query")}, failure_class="semantic_mismatch")
    ...
    def verify_query(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        return verification(expected, observed, "query", "browser verifier observed the requested search query", "browser verifier did not observe the expected search query")
    ...
    def verify_identity(payload: dict[str, Any], context: ToolExecutionContext) -> ToolResult:
        ...
        observed = str(context.state.get("observed_url") or "") or str(verifiers.observation_for("browser_url_opened").get("opened_url") or "")
        resolved = str(context.state.get("resolved_url") or context.state.get("official_result_url") or getattr(context.environment, "browser_site_catalog", {}).get(name.lower(), "") or "")
        accepted = bool(observed) and bool(resolved) and observed == resolved
        return ToolResult(status="succeeded" if accepted else "failed", message="browser verifier observed the resolved goal page identity" if accepted else "browser verifier did not observe the resolved goal page identity", evidence={"target_name": name, "observed_url": observed, "resolved_url": resolved}, accepted=accepted, failure_class="acceptance_failed" if not accepted else "none")
...
def verification(expected: str, observed: str, field: str, success: str, failure: str) -> ToolResult:
    ...
    return ToolResult(status="succeeded" if accepted else "failed", message=success if accepted else failure, evidence={f"expected_{field}": expected, f"observed_{field}": observed}, accepted=accepted, failure_class="acceptance_failed" if not accepted else "none")
