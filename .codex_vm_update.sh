...
failed=/home/rand0mg/vibeos-goal01-failed-20260715
archive=/tmp/vibeos-goal01-worktree.tar.gz
...
exec >/tmp/vibeos-goal01-update.txt 2>&1
...
rollback() {
  ...
  if [[ -d "${repo}" && ! -e "${failed}" ]]; then
    mv "${repo}" "${failed}"
  fi
  if [[ -d "${failed}/.venv" && ! -e "${backup}/.venv" ]]; then
    mv "${failed}/.venv" "${backup}/.venv"
  fi
...
echo "UPDATE_STARTED=$(date --iso-8601=seconds)"
[[ -f "${archive}" ]]
[[ -d "${repo}/.git" ]]
...
vibe doctor --json
echo "UPDATE_COMPLETED=$(date --iso-8601=seconds)"
