# Goal 05：扩展 API/CLI 用户态能力并建立可安装 Runtime

- 阶段：05 / 09
- 依赖：[Goal 04](04_system_service_recovery_vertical_slice.md)全部完成
- 风险：中高
- 完成后进入：[Goal 06](06_privileged_canary_and_rollback.md)
...
预期 Goal 04 已证明 user service 诊断/恢复纵向链路，并交付最小 Model Gateway、
Keyring provider key、按需 service facts 和一个 E0/E1 provider。开始前现场确认：

- Goal 03 的兼容矩阵、19 个 capability 和所有外部入口仍通过；
- Goal 04 哪些 schema/provider/collector 真正被两个以上场景复用；
...
后能够运行 daemon、CLI、数据库迁移和上述任务。完整升级回滚与扩展交付留到 Goal 09。
...
2. **从任务抽象最小 Machine State**
   - 只新增场景实际需要的 `MachineFact` 类型、collector、TTL、source、confidence、
     sensitivity 和 evidence reference；
   - 支持按 type/subject/task 查询、stale 标记、失效和最近变化，不保存任意文件正文；
   - Context Router 只根据固定 fact query 和
     [系统框架的数据等级](../../product/agent_system_framework.md)生成最小 manifest；
   - 不建立“三类事实必须齐全”等人为平台指标，不做本地模型基准或长期用户画像。

3. **最小 ActionSpec/Provider**
   - 为固定任务定义 action、strict parameters、effect、resource scope、precondition、
     timeout、idempotency、receipt、verify 和 reconciliation；
...
- [ ] 旧 Registry 未被批量删除，新旧接点、owner 和未来删除门禁有记录；
...
- 首批任务清单与黄金场景；
- 最小 MachineFact/Context manifest 和 ActionSpec/Provider contracts；
